...
def _run_args(ruby, script=None, extra_args=[], bundle=None, *script_args):
...
def _run_file(ruby, script, outputs, bundle=None, progress_message=None):
  tools = depset(
      transitive = [
          #ruby.toolchain.bundler.files,
          #ruby.toolchain.bundler[DefaultInfo].data_runfiles.files,
